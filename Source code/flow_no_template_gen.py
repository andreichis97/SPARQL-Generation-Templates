...
def generate_sparql_query(user_natural_language_query):
    chat_messages = [
        {"role": "system", "content": f"""You are an expert in generating SPARQL queries optimized for querying knowledge graphs. You specialize in generating SPARQL queries strictly adhering to a given ontology.
        Task:
        ...
        Step 2: Generate a SPARQL query fit to the user intent and the ontology. When generating the SPARQL query do not add prefixes.
        Step 3: Validate that the generated SPARQL query maintains logical consistency, adheres to best practices, and accurately retrieves the intended information.
    ...
    with open("./logs/no_templates_flow_logs_10_03.txt", "a") as f:
        f.write("######################################\n")
...
def respond_to_user_query(user_natural_language_query):
    generated_sparql_query = generate_sparql_query(user_natural_language_query)
...
def ragas_analyze():
    ...
    df = result.to_pandas()
    df.to_csv("./ragas_results/10_03_res/results_no_templates_flow_10_03_v5.csv", index=False)
